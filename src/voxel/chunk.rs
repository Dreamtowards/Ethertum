...
pub mod lighting {
    ...
    pub fn compute_voxel_light(queue_add: &mut VoxLightQueue, queue_del: &mut VoxLightQueue) {
        ...
        while let Some((chunkptr, local_idx, light)) = queue_add.pop() {
            ...
            // try_spread_light(&chunkptr, ivec3(x-1,y,z), lightlevel, queue_add, queue_del);
            // try_spread_light(&chunkptr, ivec3(x+1,y,z), lightlevel, queue_add, queue_del);
            // try_spread_light(&chunkptr, ivec3(x,y-1,z), lightlevel, queue_add, queue_del);
            // try_spread_light(&chunkptr, ivec3(x,y+1,z), lightlevel, queue_add, queue_del);
            // try_spread_light(&chunkptr, ivec3(x,y,z-1), lightlevel, queue_add, queue_del);
            // try_spread_light(&chunkptr, ivec3(x,y,z+1), lightlevel, queue_add, queue_del);
        }
    ...
    fn try_spread_light(chunk: &ChunkPtr, lp: IVec3, lightlevel: u16, queue_add: &mut VoxLightQueue, queue_del: &mut VoxLightQueue) {
        ...
        if  vox.light.red() < lightlevel-1 && lightlevel > 0 && !vox.is_obaque_cube() {
